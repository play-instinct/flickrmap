...
function plotMapPoints(lat, long){
map.flyTo({
                center: [long, lat,],
                zoom: 12,
    });
setTimeout(
    function() {
    let smaller_sample = state.geojson.features.slice(0, 45);
    console.log(smaller_sample)
    smaller_sample.forEach(function(marker) {
  ...
  // make a marker for each feature and add to the map
    new mapboxgl.Marker(el)
      ...
      .setHTML(`<h3> ${marker.properties.name} </h3> <div class="img-container"><img src="${marker.properties.thumb_url}"><span class="more-link">MORE INFO</span></div>`))
